...
app.post('/danhmuc/khachhang-update', function(req, res){
    ...
    con.getConnection(function(err){
        ...
        var sql = "UPDATE customer SET customerId=?, customerName=?, taxId=?, customerType=?, email=?, phone=?, address=? ";

        con.query(sql, [customerId, customerName, taxId, customerType, email, phone, address], function(err, result){
            if(err) console.log(err);
            res.redirect('/danhmuc/khachhang');
        });
    });
