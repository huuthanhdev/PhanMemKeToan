...
// Route để xử lý yêu cầu AJAX và trả về tên khách hàng dựa trên customerId
app.post('/getCustomerName', function(req, res) {
    var MaKH = req.body.customerId;
  
    // Thực hiện các thao tác truy vấn cơ sở dữ liệu của bạn để lấy tên khách hàng tương ứng với customerId

    // Ví dụ:
    var customerName = "SELECT TenKH FROM khachhang WHERE MaKH = ?" // Thay bằng logic của bạn để lấy tên khách hàng từ cơ sở dữ liệu
    con.query(customerName, [MaKH], function(err, result){
        if(err) console.log(err);
            // Gửi dữ liệu về cho trang EJS dưới dạng JSON
...
// -------------- 3.1. Sổ cái TK ----------------
app.post('/sosach/socaitk-preview',function(req, res){

    var date1 = req.query.dateFrom;
    var date2 = req.query.dateTo;
    con.connect(function(err){
        if(err) console.log(err);
        ...
        "JOIN khachhang ON hddv.MaKH = khachhang.MaKH "+
        "JOIN dichvu ON ct_phieu.MaDV = dichvu.MaDV "+
        "WHERE hddv.NgayCT >= ? AND hddv.NgayCT <=? ";
        // "WHERE MaTK =? AND NgayCT >= ? AND NgayCT <=? ";

        var SoCT = req.query.bill;
        var MaTK = req.query.sctk;
        con.query(sql, [MaTK, date1, date2], function(err, result){
            if(err) console.log(err);
            res.render(__dirname+"/assets/views/printSoCaiTK",{socaitk:result})
        });
    });
    // res.render(__dirname+'/assets/views/printBill',{bill:result});
});
...
// {R - CRUD} Load DB -> Hiển thị dịch vụ trên web
app.get('/sosach/socaitk', function(req, res){
    con.connect(function(err){
        if(err) console.log(err);
        ...
        con.query(sql, function(err, result){
            ...
            res.render(__dirname+"/assets/views/soCaiTK",{socaitk:result});
        });
    });
});
...
// -------------- 3.2. Sổ chi tiết TK ----------------
// {R - CRUD} Load DB -> Hiển thị dịch vụ trên web
app.get('/sosach/socttk', function(req, res){
    con.connect(function(err){
        if(err) console.log(err);
        ...
        con.query(sql, function(err, result){
            ...
            res.render(__dirname+"/assets/views/soCTTK",{socttk:result});
        });
    });
});


// -------------- 3.1. BÁO CÁO TỔNG HỢP DOANH THU ----------------
// {R - CRUD} Load DB -> Hiển thị dịch vụ trên web
app.get('/baocao/tonghopDT', function(req, res){
    con.connect(function(err){
        if(err) console.log(err);
        ...
        con.query(sql, function(err, result){
            ...
            res.render(__dirname+"/assets/views/baocao/tonghopDT",{baocaoTH:result});
        });
    });
});
app.post('/baocao/tonghopDT-preview',function(req, res){
    con.connect(function(err){
        if(err) console.log(err);
        ...
        con.query(sql, [SoCT], function(err, result){
            if(err) console.log(err);
            res.render(__dirname+"/assets/views/printBCTH",{baocaoTH:result})
        });
    });
    // res.render(__dirname+'/assets/views/printBill',{bill:result});
});
