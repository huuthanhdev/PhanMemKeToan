...
app.post('/danhmuc/khachhang-update', function(req, res){
    ...
    con.getConnection(function(err){
        ...
        var sql = "UPDATE customer SET customerName=?, taxId=?, customerType=?, email=?, phone=?, address=? ";

        con.query(sql, [customerName, taxId, customerType, email, phone, address], function(err, result){
            if(err) console.log(err);
            res.redirect('/assets/views/customer');
        });
    });
});
