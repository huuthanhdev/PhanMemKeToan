...
app.get('/danhmuc/taikhoan-update', function(req, res){
    ...
    con.connect(function(err){
        ...
        con.query(sql, [MaTK], function(err, result){
            if(err) console.log(err);
            res.render(__dirname+"/assets/views/taikhoan",{account:result})
        });
    });
});
app.post('/danhmuc/taikhoan-update', function(req, res){
    ...
    con.connect(function(err){
        ...
        var sql = "UPDATE dichvu SET MaTK=?, TenTK=?, =?, CapTK=? WHERE MaTK=?";

        con.query(sql, [MaTK, TenTK, CapTK, MaDV], function(err, result){
            if(err) console.log(err);
            res.redirect('/danhmuc/taikhoan');
        });
    });
